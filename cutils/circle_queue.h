...
clsCircleQueue<Type>::clsCircleQueue(uint32_t iSize) {
  ...
  assert(m_ptElt != nullptr);
}

template <typename Type>
clsCircleQueue<Type>::~clsCircleQueue() {
  free(m_ptElt), m_ptElt = nullptr;
}
...
void clsCircleQueue<Type>::Resize(uint32_t iSize) {
  ...
  if (m_ptElt != nullptr) free(m_ptElt), m_ptElt = nullptr;
...
template <typename Type>
int clsCircleQueue<Type>::Back(Type *ptElt) {
  assert(m_iSize != 0);
...
#define CIRCLEQ_INIT(type, head)        \
  do {                                  \
    (head)->cqh_first = (type *)(head); \
    (head)->cqh_last = (type *)(head);  \
  } while (0)

#define CIRCLEQ_ENTRY_INIT(elm, field) \
  do {                                 \
...
#define CIRCLEQ_INSERT_HEAD(type, head, elm, field) \
  do {                                              \
...
#define CIRCLEQ_INSERT_TAIL(type, head, elm, field) \
  do {                                              \
...
#define CIRCLEQ_REMOVE(type, head, elm, field)                       \
  do {                                                               \
